...
// TestGetAllCDN tests the function GetAllCDN
func TestGetAllCDN(t *testing.T) {
        if testing.Short() {
                t.Skip("Skipping test in short mode")
        }
        ctx, cancel := context.WithTimeout(context.Background(), 100*time.Second)
        ...
        for key, subsID := range subscriptions {
                Ldb, err := GetAllCDN(subsID)
                if err != nil {
                        t.Errorf("Failed to get CDN for subscription: %s because %v", key, err)
                }
                t.Logf("Found %d CDN in %s", len(Ldb), key)
        }
}
