...
// GetAllVPCInstances returns a complete list of instances for a given session
func GetAllVPCInstances(sess *session.Session) ([]*ec2.Vpc, error) {
	ec2c := ec2.New(sess)
	allInstancesDone := false
	var allInstances []*ec2.Vpc
	...
	for !allInstancesDone {
		...
		b.Reset()
		for _, reservation := range result.Vpcs {
